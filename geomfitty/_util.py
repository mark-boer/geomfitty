...
def distance_plane_point(plane_point, plane_normal, point):
    ...
    assert np.allclose(np.linalg.norm(plane_normal), 1.0, rtol=1e-12, atol=1e-12, equal_nan=False)
...
def distance_line_point(line_point, line_direction, point):
    ...
    assert np.allclose(np.linalg.norm(line_direction), 1.0, rtol=1e-12, atol=1e-12, equal_nan=False)
    delta_p = point - line_point
    return distance_point_point(delta_p, np.expand_dims(np.dot(delta_p, line_direction), axis=1) @ np.atleast_2d(line_direction))
