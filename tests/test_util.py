...
def assert_float_equal(f1, f2):
    ...
    np.testing.assert_allclose(f1, f2, rtol=1e-6, atol=1e-6, equal_nan=False)


def assert_vector_equal(v1, v2):
    np.testing.assert_allclose(v1, v2, rtol=1e-6, atol=1e-6, equal_nan=False)
...
def assert_direction_equivalent(v1, v2):
    ...
    assert np.allclose(v1, v2, atol=1e-6, equal_nan=False) or np.allclose(
        v1, -v2, atol=1e-6, equal_nan=False
    )
